...
def _render_primary(
    ...
    if not references or not all(path.is_file() for path in source_paths):
        limitation = (
            "Retained PBMC primary renders are materialized references because the "
            "complete compact Figure 3 source contract is not packaged in this unit."
        )
        return (
            [],
    ...
    reference_by_name = {path.name: path for path in references}
    main_refs = [
        reference_by_name[f"figure_3_pbmc_main.{suffix}"] for suffix in ("png", "pdf", "svg")
    ]
    main_outputs = _figure_outputs(output_root, unit_root, main_refs)
    figure = draw_main_figure(sources)
    _save_figure_formats(
        figure,
        {
            path.suffix.lstrip("."): main_outputs[path.relative_to(unit_root).as_posix()]
            for path in main_refs
        },
        dpi=PANEL_RASTER_DPI,
        bbox_inches=None,
    )
    plt.close(figure)
    for letter in PANEL_SET:
        panel_refs = [
            reference_by_name[f"figure_3_pbmc_panel_{letter.lower()}.{suffix}"]
            for suffix in ("png", "pdf", "svg")
        ]
        panel_outputs = _figure_outputs(output_root, unit_root, panel_refs)
        figure = draw_panel_figure(sources, letter)
        _save_figure_formats(
            figure,
            {
                path.suffix.lstrip("."): panel_outputs[path.relative_to(unit_root).as_posix()]
                for path in panel_refs
            },
            dpi=PANEL_RASTER_DPI,
            bbox_inches=_panel_bbox(figure, letter),
        )
        plt.close(figure)
    ...
    return output_paths, rows, source_paths + references, []
...
def _render_compatibility(
    ...
    packaged_dimensions, packaged_pixels, _, _ = _png_comparison(references[0], output)
    if not packaged_dimensions or not packaged_pixels:
        raise PBMCReleaseRegenerationError(
            "Compatibility current-code rerender differs from its packaged current-code reference."
        )
    ...
    row = {
        ...
        "comparison": "author-approved current-code versus retained-canonical difference",
        "reference_sha256": "",
        "regenerated_sha256": _sha256(output),
        "dimensions_match": dimensions_match,
        "byte_exact": byte_exact,
        "pixel_exact": pixel_exact,
        "pixel_difference_fraction": None,
        "maximum_channel_difference": None,
        "structural_match": dimensions_match,
        "status": "expected_current_code_render_difference",
    }
    limitation = (
        "The compatibility figure is a genuine current-code rerender. Its dimensions "
        "match the retained canonical figure, but its bytes and pixels intentionally "
        "differ because current code uses variant-local color normalization; this "
        "difference is author-approved in the packaged render audit."
    )
...
def _render_matched_reference(
    ...
    if panel_reference.is_file() and by_seed is not None:
        ...
        _draw_panel_c(figure, by_seed)
        panel_output = output_root / "artifacts" / panel_reference.relative_to(unit_root)
        _save(figure, {"png": panel_output}, bbox=_panel_bbox(figure, "C"))
        plt.close(figure)
    ...
    if materialized:
        ...
        limitations.append(
            "Some matched-reference renders remain materialized references because "
...
def _render_parameter_and_calibration(
    ...
    s7_png = s7_root / "figure_s7_pbmc_calibration.png"
    strict_references = [path for path in generated_references if path != s7_png]
    generated_paths, rows = _compare_render_set(
        unit_root=unit_root,
        output_root=output_root,
        references=strict_references,
    )
    if s7_png in generated_references:
        s7_output = output_root / "artifacts" / s7_png.relative_to(unit_root)
        s7_row = _render_comparison(
            reference=s7_png,
            regenerated=s7_output,
            artifact=s7_png.relative_to(unit_root).as_posix(),
        )
        near_exact = (
            bool(s7_row["dimensions_match"])
            and float(s7_row["pixel_difference_fraction"]) <= 1.0e-6
            and int(s7_row["maximum_channel_difference"]) <= 1
        )
        if s7_row["status"] == "fail" and not near_exact:
            raise PBMCReleaseRegenerationError(
                "PBMC S7 compact-source render exceeds its disclosed pixel-roundtrip criterion."
            )
        if s7_row["status"] == "fail":
            s7_row["comparison"] = "compact-source CSV roundtrip structural/pixel"
            s7_row["status"] = "limitation"
        rows.append(s7_row)
        generated_paths.append(s7_output)
    materialized = [path for path in references if path not in generated_references]
    limitations: list[str] = []
    if any(row["status"] == "limitation" for row in rows):
        limitations.append(
            "The S7 PNG is genuinely rerendered from packaged CSV source tables. "
            "CSV round-trip precision changes at most 1e-6 of pixels by one color "
            "level, so it is disclosed as structurally equivalent but not pixel exact."
        )
    if materialized:
        rows.extend(_materialized_render_rows(unit_root, materialized))
        inputs.extend(materialized)
